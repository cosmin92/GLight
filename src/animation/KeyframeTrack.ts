...
export default class KeyframeTrack {
    ...
    protected readonly _renderable: Renderable;
    protected readonly _controlPoints: Keyframe[];
    protected _currentKeyframeIndex: number;
    protected _interpolationType: INTERPOLATION;
    protected _minTime: number;
    protected _maxTime: number;
    protected _t: number;
    protected _point: number[];

    protected _bSpline: BSpline | null;
     ...
     */
    public constructor(renderable: Renderable, interpolationType: INTERPOLATION = INTERPOLATION.LINEAR) {
     ...
     * @param {EASE} ease
     */
    public add(time: number, controlPoint: number[], ease: EASE = EASE.LINEAR) {
     ...
     * @param {number}time
     */
    public remove(time: number): Keyframe | null {
        for (let i = 0; i < this._controlPoints.length; i++) {
            if (this._controlPoints[i].time === time) {
    ...
    public update(elapsedTime: number) {
        if (elapsedTime < this._minTime || elapsedTime > this._maxTime || this._controlPoints.length < 2) return;
     ...
     */
    public set interpolationType(type: INTERPOLATION) {

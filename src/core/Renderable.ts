...
export default class Renderable extends Object3D {
    ...
    //private static _manipulators: Manipulators = new Manipulators(); // TODO: implement it
    ...
    private _lights: LightsManager | null;

    protected _drawingMode: GLenum;

    public constructor(name: string, type: number, geometry: Geometry, material: Material) {
        super(name, type);
     ...
     * @private
     */
    private _setCameraUniforms(camera: Camera) {
     ...
     * @private
     */
    private _setLightsUniforms(lights?: LightsManager) {
        if (!lights) return;
     ...
     * @private
     */
    private _setMaterialUniforms() {

        if (this._material.type === PHONG_MATERIAL) {
            let m = <PhongMaterial>this._material;
     ...
     * @param {LightsManager} lights
     */
    public processShader(lights?: LightsManager) {
        if (lights) {
            this._lights = lights;
        }
     ...
     */
    public offScreenRender(camera: Camera, program: Program): void {
     ...
     * @param {GLenum} value
     */
    public set drawingMode(value: number) {
